...
//var testingCSV = haven't set this one up yet
...
function createBargraph(dataset){ //This was a test of D3
    ...
    for(j = 0; j < goal.length; j++){
        if (dataset[i]===0&&dataset[i+1]===0&&dataset[i+2]===0){
            ...
            var line = {Goal: goal[j],Competence: (dataset[i]-2.5),Autonomy: (dataset[i+1]-2.5),Relatedness: (dataset[i+2]-2.5)};
            i = i+3;
            newCSV.push(line);
            //console.log(line);
        }
...
var goal = ["Work/Job/Career", "Home and Household", "Intimate Relationships", "Non-Intimate Relationships", "Self-Change/Self-Growth", "Learning/Education", "Health and Medical Matters", "Leisure/Recreation", "Other", "Overall"];
